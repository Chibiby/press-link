...
export default async function AdminDashboardPage() {
   ...
   * in the subtitle, read-only, so the *when* and the *by whom* stay visible on
   * the dashboard even though the control that flips the switch now lives on
   * `/admin/users` (see `Division Wide Lock` there). Null in every other state,
   * including when the flag is unreadable — there is nothing to date.
   */
        ...
        actions={
          <>
            {/* The two contest-day portals, in filled brand teal — the only filled
                ...
                The two stay a tight gap-1 group fenced off with a hairline from one
                export errand on the other side of the rule. The rule hides below `sm`,
                where the row wraps and a stray vertical line would look like a
                rendering bug rather than a divider.

                `flex-wrap` on the group is what a narrow viewport costs: two of these
                plus Export no longer fit a 360px line, and a nowrap group would push
                the page body sideways rather than stack. It has no effect at any width
                where they do fit. */}
