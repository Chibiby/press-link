...
export function EventDonut({ summary }: { summary: PerEventSummary }) {
  ...
  return (
    // A container query, not `lg:`. What decides whether the ring and the table fit
    // side by side is the width of *this panel*, and on the dashboard the panel is one
    ...
    <div className="@container" onMouseLeave={() => setActive(null)}>
      <div className="flex flex-col items-center gap-6 @2xl:flex-row @2xl:items-start">
        ...
        <div className="min-w-0 flex-1 space-y-2">
          {/* `table-fixed`, so the two number columns are reserved before the labels
              get their turn. Under auto layout the widest label set the table's
              min-content width, the table refused to shrink to the column, and
              "Entries" and "Share" were the parts that got clipped away. */}
          <table className="w-full table-fixed text-sm">
            ...
            <thead>
              <tr className="border-b border-border text-xs text-muted-foreground">
                ...
                <th scope="col" className="w-14 py-1.5 text-right font-medium">
                  Entries
                </th>
                <th scope="col" className="w-14 py-1.5 text-right font-medium">
                  Share
            ...
            <tbody>
              {summary.slices.map((slice) => (
                ...
                >
                  ...
                  <td className="py-1.5 text-right tabular-nums whitespace-nowrap text-foreground">
